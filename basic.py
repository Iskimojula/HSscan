...
def process_points(start_measurement, start_calibration,calibration_points,measurement_points,mapping):
    ...
    for adj_point in adjacent_calibrations:
        for cal_point in calibration_points:
            if distance(adj_point, cal_point) <= 0.5:                  #理论点与实际点距离
                # 如果距离小于等于0.5，则用calibration_points中的点替换
    ...
    for calib_point in updated_adjacent_calibrations:
        # 检查相邻标定点是否仍在标定点列表中，配合remove防止重复遍历
...
def fft_sub_open(image1_filepath,image2_filepath):
    ...
    image2 = cv2.imread(image2_filepath, cv2.IMREAD_GRAYSCALE)
    # # 图像相减
    # subtracted_image = cv2.subtract(image2, image1)
    # 图像相加
    subtracted_image = cv2.add(image2, image1)
...
def sph_cyl_axis(fit_list, r,X):
    r = r * X / 1000
    cly = -4 * sqrt(6) * sqrt(fit_list[3] ** 2 + fit_list[5] ** 2) / r ** 2
    sph = -4 * sqrt(3) * fit_list[4] / r ** 2 - cly / 2
    result_radians = 0.5 * math.atan(fit_list[3] / fit_list[5])
    axis = math.degrees(result_radians)
...
def calculate(img,radius):                                                   #H为True时，进行可调区域计算，否则正常计算
    ...
    center_spot=find_spot_centers(img)                        #img为滤波后的图片
    ...
    if len(contours) > 0:
        ...
        for i, number in enumerate(contours, 1):
            if cv2.contourArea(number) < Mid_Area * (1/3) or cv2.contourArea(number)>Mid_Area*2:  #比较与中间的光斑面积大小做出判别
                continue
        ...
        for num, contour in enumerate(valid_contours, 1):  # num表示下标 contours数组当括号内数字为1时第一个下标为1否则默认从0开始
            ...
            if w > h*1.5 or h > w*1.5:                     # 限制光斑长宽比例，以有效去除杂光
                continue
            sum_1 = 0
            sum_2 = 0
            sum_3 = 0
        ...
        #print("经过筛选的光斑个数",len(centroid_list))
...
#计算平均斜率
def averageslope(xy_diffs,radius,F,X):#输入radius是像素
    j=[]
    for i in xy_diffs:
        x=i*X #像素转距离，单位um
        f=F*1000 #单位转换，单位um
...
# 定义子函数用于绘制连线
def draw_lines(canvas, list_standard, list_nostandard,point):
    ...
    # cv2.imwrite('C:\\Users\\Administrator\\Desktop\\456\\low.bmp', canvas)
    #标记第一个点位置
    cv2.circle(canvas, point, radius=5, color=(0, 255, 0), thickness=-1)
...
def lvpyfun(image1_filepath,image2_filepath,radius,F,X,standard_spots_filepath,n):       #图片的地址，画圆半径单位毫米，透镜阵列焦距单位毫米，一个像素几微米，标准点坐标
    ...
    # 把毫米单位半径转化为像素单位半径
    radius = round(radius * 1000 / X) #像素半径
    # 读入标准点坐标的的内容

    central2=read_function(standard_spots_filepath)
    #每个窗口采用不同阈值
    subaperture_centers=central2
    final_image=adaptive_threshold(filtering, subaperture_centers, (87,87)) #87 窗口的像素值
    cv2.imshow("final_image",final_image)

    # 去底噪
    final_image = cv2.subtract(final_image, 10) #
    ...
    if len(mapping)==0:    #说明外推法失效，可能起始点周围子孔径无测量点
        print("起始点相邻子孔径无测量点，外推法失效")
        flag=False
    else:
        standard_points = []
        ...
        # #调用连线函数
        start_point=[]
        start_point.append(round(initial_calibration[0]))
        start_point.append(round(initial_calibration[1]))
        draw_lines(canvas_picture, standard_points, no_standard_points_ratated,start_point)
    ...
    if flag:
        ...
        # # 显示框框图
        # cv2.imshow("10",img10)
        # cv2.imshow("img10",img11)
        #给入可调区域的偏移坐标central1与完整标准点的central2，xy_diffs计算出的是像素之差
        ...
        #计算球镜度，柱镜度数，轴向
        sph,cly,axis=sph_cyl_axis(r_list,radius,X)
        r_list.append(sph)
        r_list.append(cly)
        r_list.append(axis)
        return r_list
...
def ellipsefun(image1_filepath,image2_filepath,radius,F,X,standard_spots_filepath,n,Mx,My):
    #对图片进行滤波处理，图片2-1
    filtering=fft_sub_open(image1_filepath,image2_filepath)
    # color_image = cv2.cvtColor(filtering, cv2.COLOR_GRAY2BGR)
    # # 读入图片的地址
    ...
    # 读入标准点坐标的的内容
    central2=read_function(standard_spots_filepath)
    #central2=opt.calc_std_spots(standard_spots_filepath,Mx,My)
    #每个窗口采用不同阈值
    subaperture_centers=central2
    final_image=adaptive_threshold(filtering, subaperture_centers, (87,87)) #87 窗口的像素值
    cv2.imshow("final_image",final_image)
    ...
    if flag:
        ...
        # cv2.imshow("img10",img11)
        #给入可调区域的偏移坐标central1与完整标准点的central2，xy_diffs计算出的是像素之差
        xy_diffs,list_standard_spots=opt.calc_xydiff(no_standard_points_ratated,standard_points,Mx,My)
        #print(xy_diffs)
        #给如偏移量，计算平均斜率
        J=opt.calc_J(xy_diffs,radius,F,X)
        #给如标准点坐标，计算D矩阵
        D=opt.calc_D(no_standard_points_ratated,radius,n,center,Mx,My)
        #计算系数C与保留三位小数
        C=compute_C(D,J)
        rounded_coefficients = np.round(C, decimals=3)*(-1)
        r_list = list(rounded_coefficients)
...
#1.搜寻范围改为50 2.底噪自定义
#labview暂时还未添加 #3.去除严重比例失调窗口 4.起始点周围子孔径无测量点的报错问题修正
